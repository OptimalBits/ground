                    ...
                    if(!err) {
                        _.each(oldItems, function (item) {
                            if(!item.__op || item.__op === 'insync') {
                                console.log('del');
                                console.log(item);
                                storage.deleteItem(keyPath, item.keyPath, {
                                    insync: true
                                }, function (err) {
                                    console.log('deleted');
                                });
...
_.last(keyPath)                ];
            ...
            Queue.prototype.next = function (keyPath, refItemKeyPath, opts, cb) {
                var _this = this;
                this.localStorage.next(keyPath, refItemKeyPath, opts, function (err, item) {
                    console.log('------1');
                    if(item) {
                        cb(err, item);
                    } else {
                        ...
                        _this.remoteStorage.next(keyPath, refItemKeyPath, opts, function (err, itemRemote) {
                            console.log('-----2');
                            if(itemRemote) {
                                _this.localStorage.insertBefore(keyPath, null, itemRemote.keyPath, opts, function (err) {
                                    cb(err, itemRemote);
                                });
                            } else {
                                cb(err, itemRemote);
            ...
            Queue.prototype.deleteItem = function (keyPath, itemKeyPath, opts, cb) {
                var _this = this;
                this.localStorage.deleteItem(keyPath, itemKeyPath, opts, function (err) {
                    if(!err) {
                        _this.addCmd({
                            cmd: 'deleteItem',
                            keyPath: keyPath,
                ...
                if(!err) {
                    ...
                    this.dequeueCmd(function (err) {
                        ...
                        switch(cmd.cmd) {
                            ...
                            case 'insertBefore': {
                                storage.set(cmd.keyPath, cmd.itemKeyPath, function (err) {
                                    Gnd.Util.nextTick(syncFn);
                                });
                                break;
